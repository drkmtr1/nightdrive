...
const ENTRY = resolve("src/evaluation/stage8-motif-reference-primitives.ts");
...
describe("Stage 8 Motif reference primitive independence", () => {
  ...
  it("loads and executes primitive operations while production Motif modules and calls are blocked", async () => {
    ...
    try {
      const reference = await import("./stage8-motif-reference-primitives");
      expect(reference.deriveStage8MotifReferenceSeed(0)).toBe(3_256_624_460);
      expect(reference.createStage8MotifReferencePrng(0).nextUint32()).toBe(1_144_304_738);
      expect(
        reference.selectStage8MotifReferenceWeightedCandidate(
          [
