...
The next eligible gate is a separately authorized SPECIFY task for the bounded Energy/Complexity policy-design investigation required before further listening: determine whether an immutable profile-data version using the existing selection algorithm or a narrowly scoped policy-semantics/decision-ownership version is warranted. Do not infer weights or freeze an evaluation protocol in this snapshot. Matched V1/R1 comparative listening remains later and separately gated; the baseline remains exploratory, R1-REV-001 remains partially closed, aggregate provenance remains later, and Stage 8 is not authorized.
