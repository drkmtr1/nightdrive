...
Review the proposed ADR-021 specification. If accepted, the next separately authorized SPECIFY task is matched V1/R1 comparison design under the existing evaluation contracts, using unchanged R1; no protocol, seeds, artifacts, tuning, or listening are authorized here. The baseline remains exploratory, R1-REV-001 remains partially closed, aggregate provenance remains later, MIA-003 remains deferred/non-blocking, and Stage 8 is not authorized.
