...
- When proposing [routine dedicated-reviewer substitution](../AI_ENGINEERING_WORKFLOW.md#routine-dedicated-reviewer-substitution), record why this exact task meets every eligibility condition and no exclusion, the qualified reviewer configuration, separate read-only invocation, prior-finding search, and fallback to the existing independent reviewer. Omission or uncertain eligibility bars substitution.
