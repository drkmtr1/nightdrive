...
- Exact-head review handoff: include candidate tree SHA and applicable OPEN finding IDs with durable review/result source, origin tuple, affected paths/blobs where practical, and current resolution evidence. Identify governing accepted contracts independently of candidate tests. Request the [contract-first playbook result](../ENGINEERING_REVIEW_PLAYBOOK.md#contract-first-exact-head-review); a separate Codex review remains preliminary unless accepted policy grants substitution.
- When proposing [settled-contract dedicated-reviewer substitution](../AI_ENGINEERING_WORKFLOW.md#settled-contract-dedicated-reviewer-substitution), record why the governing authority is accepted and sufficient, why the candidate only implements it, every eligibility condition, every applicable external-review boundary, the qualified reviewer configuration, separate read-only invocation, prior-finding search/disposition, and fallback route. Risk or implementation-category labels do not decide eligibility; omission or uncertainty bars substitution.
