...
| AC-039 | MUS-012 | The documented V1 `ChordQuality` contract contains exactly major triad `major-triad` `[0,4,7]`, minor triad `minor-triad` `[0,3,7]`, and diminished triad `diminished-triad` `[0,3,6]`; each formula is tonic-relative, integer, unique, ascending, and membership-only. Root, ordering, inversions, voicings, spelling, extensions, and seventh identities are outside the quality; seventh structures are explicitly deferred extension metadata. No production ChordQuality or Chord implementation is included. |
