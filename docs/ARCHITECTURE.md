...
The separately versioned First Playable composition boundary is the next broader deterministic composition capability after Stage 7. Its accepted `generators` coordinator validates a normalized request, realizes Harmony once, then invokes existing Bass and public V2 Arpeggiator boundaries with that same realization; the implementation is accepted and merged through PR #175. A `composition` value module owns the resulting canonical projection, validation, serialization, and hash semantics. Pinned-Node/cross-runtime canonical evidence remains separately gated. This does not change the supplied-Harmony-only Stage 7 aggregate, and application/UI/audio adapters remain downstream consumers.

The Stage 8 V1 [Motif contract](MOTIF_MODEL.md) is accepted through PR #217. Its immutable rhythm/contour catalogs, profile-data/list construction, internal four-draw policy resolver, internal deterministic pitch projector, canonical result/provenance boundary, and public Motif generation operation are integrated through PRs #219, #220, #222, #224, #226, and #228. Immutable Motif values and invariants remain in `music-domain`; supplied-Harmony policy resolution, exhaustive pitch projection, and public request orchestration remain in `generators`. Harmony remains authoritative and is never regenerated or modified. Later composition integration remains unimplemented; the accepted boundary neither extends the First Playable schema nor creates an application, browser, audio, MIDI, persistence, or AI boundary.
