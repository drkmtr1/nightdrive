...
The separately versioned First Playable composition boundary is the next broader deterministic composition capability after Stage 7. Its future `generators` coordinator validates a normalized request, realizes Harmony once, then invokes existing Bass and public V2 Arpeggiator boundaries with that same realization. A `composition` value module owns the resulting canonical projection, validation, serialization, and hash semantics. This does not change the supplied-Harmony-only Stage 7 aggregate, and application/UI/audio adapters remain downstream consumers.
