...
The Stage 8 V1 [Motif contract](MOTIF_MODEL.md) is a documentation candidate under consequential review; no Motif runtime exists. It keeps immutable Motif values and invariants in `music-domain` and supplied-Harmony policy resolution/projection in `generators`. Harmony remains authoritative and is never regenerated or modified. The boundary neither extends the accepted First Playable schema nor creates an application, browser, audio, MIDI, persistence, or AI boundary; later composition integration requires a separately versioned contract.
