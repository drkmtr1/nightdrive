...
The accepted [Stage 7 Arpeggiator audition-artifact contract](reviews/STAGE7_ARPEGGIATOR_AUDITION_ARTIFACT.md) defines an evaluation-only adapter from accepted Harmony plus Arp outputs to the existing Nightdrive MIDI IR. The local `src/evaluation/stage7-arpeggiator-midi-ir.ts` implementation is validation-complete and review-pending; it stops at `MidiIr`, remains noncanonical and outside the public music-domain API, and depends downstream on `src/music-domain` outputs and `src/midi`. No dependency flows back into the music domain. It introduces no second MIDI schema/serializer, aggregate provenance, browser/Web Audio path, application route, or persistence boundary. Serialization, fixtures, and listening remain separately gated.
