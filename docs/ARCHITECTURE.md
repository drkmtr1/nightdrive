...
Stage 7C separates deterministic Arpeggiator policy resolution from canonical event projection. The enclosing generator derives a component-isolated Arpeggiator seed from the composition root seed and a stable named component identity, then the policy resolver uses one versioned Mulberry32 stream to resolve bounded choices into a structured Arp plan. The event projector consumes that plan without becoming a random-note generator. Harmony remains authoritative for selected voicings; component-seed derivation, policy, profile, generator, and schema versions remain replay boundaries; aggregate provenance remains outside `ArpEvent`. Stage 7C1 mask, Stage 7C2 weighted-choice, and Stage 7C3 component-seed derivation contracts are accepted and merged, with Stage 7C3 recorded through PR #75 (approved head `81b3c878daed262e18a50a2a539235c7bbc7e772`, merge commit `ec90258658a789d186f297cbc7041983bbbbea8a`). Stage 7C-P1 is accepted and merged through PR #79, and Stage 7C4 is accepted and merged through PR #80. Stage 7C5 freezes structured-error ownership and precedence without changing this boundary; Stage 7C runtime APIs remain separately gated.
