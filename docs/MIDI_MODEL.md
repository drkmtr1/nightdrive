...
The bounded flow is canonical composition/timing → Nightdrive IR → isolated SMF adapter → binary `.mid` → browser delivery adapter. The Stage 5D1 adapter owns download mechanics only; server delivery, package workflows, and full export UX remain later work and do not alter the IR.
...
For the declared tested environment, FL Studio Producer Edition 2025 `26.1.6.5639`, the Stage 5C1 fixture was imported into a fresh blank project using the recorded options in `docs/reviews/STAGE5_FL_STUDIO_INTEROPERABILITY.md`. The clean re-import passed file loading, tempo, meter, alignment, eight-bar length, track separation/names, pitch/timing/duration checks, the tick-30720 boundary note, and playback/no-missing/no-unexpected-note observations. Exact numeric velocity preservation was not directly verifiable from the collected UI evidence. This satisfies MIDI-004 for that environment only; it does not claim compatibility with other FL Studio versions, macOS, other DAWs, or other option combinations.
...
Stage 5C1 commits one readable eight-bar fixture at `src/midi/fixtures/stage-5c1-interoperability.mid`, generated by `serializeStandardMidiV1` from the adjacent source IR. Its SHA-256 is `230d6c7eb67eaa512b61f381d6255276f8b575367ee9bc4c634e1f94eb53ce2d`. Automated tests regenerate identical bytes and use the Stage 5B2b independent reader to verify Format 1/960, 120 BPM, 4/4, fixed component tracks/channels, note semantics, explicit Note Off velocity `0`, and terminal tick `30720`. The manual import and inspection protocol is `docs/reviews/STAGE5_FL_STUDIO_INTEROPERABILITY.md`; its recorded clean re-import satisfies MIDI-004 for FL Studio Producer Edition 2025 `26.1.6.5639`, with exact numeric velocity preservation not directly verifiable. Broader compatibility remains unproven.

## Stage 5D1 browser MIDI delivery

The Stage 5D1 browser adapter accepts already-serialized `Uint8Array` MIDI bytes and a caller-provided logical basename. It creates an `audio/midi` Blob, triggers a temporary anchor with a deterministic sanitized `.mid` filename, then removes the anchor and revokes the object URL. The adapter owns browser download mechanics only: it does not validate or alter MIDI semantics, import the serializer, persist bytes, make a server round trip, or maintain a global URL registry. No package/ZIP export, server route, browser export UX, or broader FL Studio compatibility claim is included.
