...
3. **Build an obligation ledger.** For each materially relevant normative clause, record source, invariant, affected path/function, inspected evidence, and `SATISFIED | VIOLATED | NOT APPLICABLE | NOT VERIFIED`; justify inapplicability or missing evidence. Include constraints and qualifying conditions as well as happy-path behavior. Any materially applicable `VIOLATED` or `NOT VERIFIED` obligation bars PASS.
4. **Audit permissions.** In qualification, validation, provenance, security, canonicalization, and other fail-closed behavior, enumerate candidate exemptions, ignored states, skips, allowlists, fallbacks, defaults to PASS, evidence-unavailable conversions, and path/category classifications that reduce inspection. For each, record its accepted authority basis and proof that its preconditions hold for the actual invocation where required. A candidate test repeating its own permission is not independent justification. Unsupported or unverified permission bars PASS.
5. **Challenge tests.** Check whether tests prove the accepted obligation or encode the implementation's assumption: circular or implementation-derived expectations, absent negative cases, fail-open paths, weak assertions, and candidate tests that redefine a requirement. Record validation actually run and its limitations. Green tests do not override accepted authority.
6. **Carry findings.** Consult durable review/result evidence for applicable OPEN findings. Record stable ID, originating base/head, affected path and Git blob where practical, contract basis, `OPEN | RESOLVED | SUPERSEDED`, and explicit closure evidence. Compare affected blobs and behavior across new commits, branches, bases, rebases, cherry-picks, and reconstructions. Materially unchanged rejected content remains OPEN; do not infer closure from a new SHA. Unavailable finding evidence is `NOT VERIFIED`, not silent closure. `PROJECT_STATE.md` holds only current coordination impact.
7. **Return a structured disposition.** Report `PASS | REVISE | BLOCKED`, identity/settings, exact base/head/tree/scope, contracts consulted, ledger and completeness, permissive-path audit, findings with severity/evidence/impact, each relevant prior-finding disposition, validation/evidence limits, and confirmation the reviewer did not modify the candidate. `REVISE` identifies demonstrated violations; `BLOCKED` identifies missing authority or evidence that prevents judgment. PASS requires complete candidate inspection, all material obligations satisfied or justified not applicable, no applicable unresolved blocking finding, and every required result field. A narrative missing required fields cannot be promoted to PASS.

Use explicit result fields even when a field is `none` or `not verifiable`: **Verdict; Reviewer/invocation/effective settings/sandbox; Base/head/tree/scope; Authority; Obligation ledger and completeness; Permissive mechanisms and their authority/invocation evidence; Findings; Prior findings and disposition; Validation/evidence limitations; Candidate modified; Review completeness.** The receiving agent checks these fields and the current tuple before using a result. A tool option that requests a schema is not proof that the reviewer actually emitted one.
