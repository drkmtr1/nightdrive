...
Provides independent exact-head review, consequential architecture and contract review, and help with genuinely unresolved blockers through an available review channel. ChatGPT does not override accepted repository contracts or routinely author every task packet. An exact-head PASS may lead to publication and merge under standing authority only when the reviewed tuple, required checks, accepted merge method, and current repository eligibility all hold.
