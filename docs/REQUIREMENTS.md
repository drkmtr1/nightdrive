...
| MUS-023 | P0 | The Stage 4B1 runtime validates the versioned concrete Harmony template catalog and profile compatibility, including optional slot inversion restrictions, then deterministically realizes explicit slots into ordered existing Chords without quality inference or later Harmony behavior. | AC-050 | 4B1 |
...
| MUS-026 | P0 | The Stage 4B4 runtime implements deterministic sequential realization of one ordered Harmony progression from validated profile/template/Key inputs, preserving slot bars, ranking already hard-valid Stage 4B2 candidates by lower Stage 4B3 adjacent cost, then applicable Stage 4B4 soft-preference rank, then the accepted Stage 4B3 pitch tie-break sequence, and propagating specific failures, without modifying the Stage 4B3 selector or performing progression-wide optimization. | AC-053 | 4B4 |
| MUS-027 | P0 | Stage 5A defines a versioned Nightdrive-owned MIDI intermediate representation mapped only from validated canonical composition/timing data and derived Harmony realizations; it preserves integer ticks, note spans, pitch, velocity, component identity, and explicit tempo/meter without making MIDI bytes canonical. | AC-054 | 5A |
...
| NFR-026 | P0 | Stage 5A freezes deterministic Standard MIDI File format 1 mapping at exactly 960 PPQ, conductor track 0 and fixed component order/channel policy, explicit `0x8n` Note Off with release velocity 0, equal-tick ordering, note-on/off expansion, exactly one End-of-Track at tick 30720 per track, structured failures, semantic-versus-binary determinism, isolated adapter boundaries, independent-parser validation, and a documented dependency-spike gate without adding a MIDI dependency or implementation. | AC-054, AC-055 | 5A |
