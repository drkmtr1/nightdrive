...
R1 is the accepted exact dataset for `nightdrive.genre-profile.arpeggiator.v2`, identified by complete research fingerprint `b6f7ee16f33cf649ae2c6f06e4b5eecf859409b1917e2bc641323857fc1956e8`. Its dataset-only acceptance preceded runtime integration, now accepted through PR #131. Current R1/V2 behavior is Product Owner accepted for continued development under the [35-fixture acceptance override](STAGE7_ARPEGGIATOR_EVALUATION_RESULTS.md#product-owner-r1v2-acceptance-and-comparison-override), not full matched-protocol completion or final Stage 7 acceptance. This research representation is not a caller-supplied configuration API or canonical persistence serialization. The completed [V1 evaluation](STAGE7_ARPEGGIATOR_EVALUATION_RESULTS.md) remains locked and its Energy and Complexity sensitivity dispositions remain REVISE/open. [ADR-019](../DECISIONS.md) and the [genre-profile model](../GENRE_PROFILE_MODEL.md) own compatibility and V1 history; this record owns R1's exact tables, fingerprint, V1 change ledger, and research qualifications. Arithmetic, hash, structure, and permitted-delta evidence were independently verified; collision totals are now reproduced by the accepted retained diagnostic merged through PR #114; historical search/finalist chronology remains unreproduced.

The accepted dataset identity is `nightdrive.genre-profile.arpeggiator.v2`, paired only with accepted policy compatibility identity `nightdrive.arpeggiator-policy.v2`. The supported historical pair remains policy V1 + profile-data V1; the two cross-version pairs are unsupported; policy V2 + profile-data V2 is implemented through PR #131. No fallback, implicit migration, `latest` alias, or V1 reinterpretation is permitted. V1 public requests, results, errors, plans, events, and locked evidence retain their original replay meaning. The distinct V2 public request/result/error contract is accepted and merged through PR #111; the public operation is accepted through PR #131.
...
## Reproducible diagnostic checkpoint (accepted and merged through PR #114)

Run `npm test -- src/evaluation/stage7-r1-diagnostics.test.ts`. The test parses the JSON above and verifies its exact accepted fingerprint before use. It reuses the existing resolver through the established test-only candidate-list mock seam, component-seed derivation, Mulberry32, weighted choice, gate mapping, golden Harmony realization, and resolved-plan projector; no production V2 operation is implemented.

The diagnostic independently reproduces all fresh-root table totals above, all nine adjacent regressions and the 28/3/9 classification, and Darkwave's 66/70 event collisions. It also reproduces the earlier five V1 Midtempo projection-collapse cases on roots `0..255`. Plan equality compares all five canonical fields; event equality compares ordered `(pitch, startTick, durationTicks)` values. The 40 adjacent relationships are two axes × five fixed other levels × four neighboring-level pairs, each over roots `1024..2047`. Each complete fresh-root enumeration runs twice with identical results and throwing ambient-randomness/clock guards. Projection is memoized only by the complete plan and fixed profile context; policy selection still runs for every request.

Executable preservation evidence covers all 40 individual medium source rows, all 20 medium/medium effective lists, all 25 Darkwave medium-Complexity Energy lists, and representative same-seed plan/event equality at these preserved settings. V1 and R1 data remain unchanged. This supersedes the earlier lack of independently reproduced collision evidence, not the qualifications about perceptual benefit.

**R1-REV-001: PARTIALLY CLOSED.** The retained diagnostic's exact frozen-candidate numerical reproduction is accepted and merged through PR #114; historical 30,000-table search execution, finalist selection, and no-retuning chronology remain unreproduced. Reproduction does not establish musical acceptance, untouched holdout evidence, or authorization for production V2 runtime.

## Later gates

The accepted V2 runtime evidence obligations are unchanged: the retained reproducible diagnostic checkpoint above remains executable evidence. Literal V2 data validation must reproduce this exact candidate structure and fingerprint, independently construct and compare all 500 lists, and prove center and Darkwave Energy anchors. Tests must preserve V1 public replay and negative-path behavior, reject both incompatible version pairs, prove the fixed five-draw schedule and deterministic replay without extra PRNG consumption, and reproduce the relevant candidate diagnostics. The exact V2 public request/result/error boundary is accepted and merged through PR #111; V2 implementation is accepted through PR #131; aggregate provenance remains separate and unimplemented.

The original matched-comparison prerequisite required the following before musical acceptance (now waived for current R1 product acceptance only by the [Product Owner override](STAGE7_ARPEGGIATOR_EVALUATION_RESULTS.md#product-owner-r1v2-acceptance-and-comparison-override)): a separately designed matched V1/R1 evaluation would use the same Harmony contexts and matched roots while preserving the original locked V1 evidence. Version identity should be blinded and counterbalanced where feasible. It would assess individual usability and Energy/Complexity comparisons across all four profiles, include intermediate low–medium and medium–high relationships, explicitly cover the disclosed Midtempo and control-profile qualifications, and avoid cherry-picking roots to hide retained collisions. The accepted protocol and frozen artifacts remain preserved; the remaining execution is superseded, not completed. Stage 7 remains open and Stage 8 unauthorized.
