...
Stage 7C4 assigns no public error code, field, or mixed-invalid precedence. Future validation must cover unsupported profile/policy versions, malformed candidate data, invalid normalized intent, invalid gate identity, impossible profile construction, and downstream invariant failure, but the exact structured-error contract remains a separately authorized follow-on slice. This section defines no runtime type, selector, resolver, octave expansion, mask application, seed derivation, or event-projection implementation.
...
The following remain separately gated for runtime implementation but are not removed from eventual Stage 7 scope: the accepted Stage 7C1 deterministic density/rest-mask contract; the accepted Stage 7C2 deterministic weighted-choice contract; the accepted Stage 7C3 component-seed derivation contract; the documented upward octave expansion; the Stage 7C4 exact profile-policy contract; seeded bounded policy resolution; and aggregate generator/provenance integration. Stage 7C public error taxonomy and precedence require a separate follow-on contract review. `alternate` and `seededRandom` direction semantics, scale-tone transforms or other non-selected-voicing pitch sources, triplets, dotted and thirty-second rates, free-running Arp, VST automation, velocity/accent, MIDI, browser/audio, UI, persistence, and AI behavior remain outside this checkpoint.

Stage 7C4 freezes exact ordered candidates and raw integer weights for Dark Synthwave, Classic Synthwave, Darkwave, and Midtempo Cyberpunk in the genre-profile model. They are versioned Nightdrive hypotheses, not universal genre claims or runtime authorization. Stage 7 cannot be declared profile-appropriate or complete until the remaining structured-error contract, separately authorized runtime implementation, deterministic evidence, and structured human listening review are accepted.
