...
The versioned `nightdrive.prng.mulberry32.v1` primitive, canonical uint32 seed/state validation, deterministic stepping, and reusable Stage 7C7a1 component-seed derivation are implemented. Stage 7C7a2 implements only the generic internal weighted-choice primitive; it receives a supplied uint32 and does not call the PRNG. No seed-consuming production Arpeggiator consumer exists, and Stage 7B has no seed input. Seeded policy resolution and the shared generator/provenance envelope remain unimplemented.
...
All Stage 7C7a5 configuration objects, validators, error types, and closed policy types remain direct-module exports only and are not re-exported from `src/music-domain/index.ts`. `ArpDensityMaskIdV1` and `WeightedCandidate<T>` remain non-barrel. Broad Stage 7C request, plan, result, and version exposure remains deferred until required by the accepted enclosing operation. Later internal Stage 7C modules may import the direct module, but callers cannot inject arbitrary shared policy configuration.
...
### Stage 7C7a6 genre-profile configuration runtime implementation

Stage 7C7a6 is the current bounded implementation/review slice. It owns only the immutable literal `nightdrive.genre-profile.arpeggiator.v1` data set, exact structural validation of that accepted data, and deterministic construction of one candidate-aligned raw weighted list. It reuses the Stage 7C7a5 policy identity and closed domains without redefining them, and it does not select a candidate. It is not accepted or merged.
...
The future Stage 7C public operation has this complete normative precedence. Steps 1–14 are preflight and consume no PRNG output:
...
Stage 7C5 freezes the taxonomy, field ownership, and two-phase precedence above, and Stage 7C6 freezes the public enclosing interface and private helper ownership. Stage 7C7a1 implements only the reusable neutral component-seed derivation boundary, Stage 7C7a2 implements only the internal generic weighted-choice primitive, Stage 7C7a3 implements only the internal immutable density-mask catalog and deterministic lookup, and accepted Stage 7C7a4 implements only the upstream shared canonical Energy/Complexity boundary consumed by the future request. Stage 7C7a5 implements only the shared policy-configuration foundation and its private `policy.version`-owned failure boundary for review. Direct shared-domain and shared-configuration errors remain distinct from the later enclosing operation's `ArpValueError` behavior. The public enclosing operation and its Arpeggiator-owned runtime behavior remain unimplemented. Future runtime work must supply the specified structured public failures, internal-assertion distinction, no-consumption preflight, no-partial-output behavior, and exact Stage 7B regression compatibility before it may claim this gate.

## Deferred Stage 7 behavior

The following remain separately gated for runtime implementation but are not removed from eventual Stage 7 scope: Stage 7C1 density/rest-mask execution beyond the Stage 7C7a3 catalog runtime; the documented upward octave expansion; the accepted Stage 7C4 exact profile-policy contract; the accepted Stage 7C5 structured-error contract; the Stage 7C6 enclosing runtime interface beyond the Stage 7C7a1 component-seed, Stage 7C7a2 weighted-choice, and Stage 7C7a3 mask-catalog primitives; seeded bounded policy resolution; and aggregate generator/provenance integration. `alternate` and `seededRandom` direction semantics, scale-tone transforms or other non-selected-voicing pitch sources, triplets, dotted and thirty-second rates, free-running Arp, VST automation, velocity/accent, MIDI, browser/audio, UI, persistence, and AI behavior remain outside this checkpoint.
...
Future Stage 7C4 evidence must enumerate every profile and policy slot to prove exact candidate membership, order, and final raw weights for all 25 energy/complexity pairs; every final list must satisfy Stage 7C2 without reordering or normalization. Independent fixtures must prove that changing only energy or only complexity leaves every documented invariant slot unchanged, that Darkwave energy changes density before register, and that Midtempo Cyberpunk complexity changes mask phase preference without equating complexity to note count. Gate fixtures must cover all nine rate/gate mappings, legal Stage 7B4 integer bounds, duration-only effects, and exact `long === rateTicks` compatibility. Replay fixtures must hold profile ID/data version, Arpeggiator policy version, normalized intent, root seed, seed-derivation version, and PRNG version constant; prove the fixed five-output schedule including the one-candidate Midtempo gate; prove component isolation; and fail if `Math.random`, wall clock, locale, network, AI output, database/discovery order, or object iteration affects resolution. Human profile-fit evaluation remains a separate acceptance gate from deterministic correctness.

Future Stage 7C5 evidence must assert every exact public code and field at its owning boundary, every valid boundary value, the complete mixed-invalid tables and precedence above, and exact Stage 7B regression compatibility. It must distinguish caller-invalid input, unsupported versions and version pairs, invalid immutable configuration, incompatible canonical context, whole-operation musical unsatisfiability, and internal assertions; prove that all five candidate lists plus shared policy/gate data, range, Harmony, and profile-context equality validate before the first PRNG output; prove all four matching profile pairs pass and all twelve ordered unequal pairs among the four V1 profile IDs fail as `INCOMPATIBLE_ARP_PROFILE_CONTEXT` at `profile.id`; prove mismatch consumes no component-seed or PRNG output; prove no partial plan or events escape any failure; and prove equal invalid input yields equal structured failure independent of ambient randomness, time, locale, network, AI, discovery, or object iteration. MIA-004 remains deferred until those separately authorized runtime assertions exist and pass.
