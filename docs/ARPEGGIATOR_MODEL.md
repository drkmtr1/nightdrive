...
The versioned `nightdrive.prng.mulberry32.v1` primitive, canonical uint32 seed/state validation, and deterministic stepping are implemented. Stage 7C6 below freezes an Arpeggiator-specific seed-bearing runtime interface, while Stage 7C7a1 implements only the reusable component-seed derivation primitive; no seed-consuming production Arpeggiator consumer exists, and Stage 7B has no seed input. Weighted choice, seeded policy resolution, and the shared generator/provenance envelope remain unimplemented.
