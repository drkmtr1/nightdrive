...
Stage 7A defines the deterministic foundation over a validated Harmony progression. Stage 7B1 implements exact selected-voicing range filtering, and accepted merged Stage 7B2 projects monophonic immutable `ArpEvent` values at fixed eighth rate, ascending direction, full-step gate, and slot-local reset. Stage 7B3 contract readiness defines a complete optional third `ArpTraversalParametersV1` argument containing only rate (`quarter|eighth|sixteenth`) and direction (`up|down|up-down|down-up`); omitting the argument preserves the Stage 7B2 defaults, while implementation remains separately gated. Inclusive absolute MIDI range remains the second argument, and configurable integer `gateTicks` remains exclusively Stage 7B4. See [Arpeggiator model](ARPEGGIATOR_MODEL.md) for normative cycles, validation, and boundary behavior.
