...
The complete Stage 8 V1 operation, identities, phrase roles, rhythm/contour catalogs, pitch projection, constraints, canonical representation, provenance, and errors are defined only in [Stage 8 melody and motif model](MOTIF_MODEL.md). V1 represents motif identity as exact rhythm/onset/duration form plus signed diatonic contour. Phrase 2 is exact `motif-form-repetition` at that identity boundary rather than literal MIDI-pitch repetition; Harmony-authoritative projection may change only concrete pitches. Phrase 3 is harmony-aware transposition. Phrase 4 preserves the contour and may displace only nonstructural events by its selected bounded value.

The contract excludes inversion, retrograde, augmentation, diminution, chromatic transformation, arbitrary LLM notes, hidden random draws, and silent policy tuning. Production implementation remains gated on consequential contract review and does not change the accepted First Playable boundary.
