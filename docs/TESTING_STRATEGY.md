...
Exact mask catalogs, integer-weighted-choice bucket mechanics, profile weights/gate candidates, seed-derivation vectors, and Stage 7C structured-error precedence require accepted follow-on contracts before tests may claim them. Automated correctness does not establish genre fit: all four V1 profiles require structured human listening evidence for distinction, Harmony support, useful repetition, intuitive energy development, complexity without noise, and seed variation that remains recognizably inside profile identity. Foundation tests must not claim full AC-004 seeded replay, seeded AC-011 behavior, or complete profile-appropriate Stage 7 acceptance before those gates pass.
