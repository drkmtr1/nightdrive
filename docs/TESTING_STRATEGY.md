...
The documentation-only straight-rhythm contract requires future automated evidence for all five closed identifiers. `sustained` must be byte-equivalent to Stage 6A2. Quarter, eighth, and sixteenth pulses must assert exact starts, durations, and event counts at `960`, `480`, and `240` ticks. `offbeat-eighth` must assert no slot-start onset, duration `480`, per-local-bar starts `480`, `1,440`, `2,400`, and `3,360`, and exact repetition in multi-bar slots. Every mode must prove that phase restarts at each Harmony slot, no event crosses a slot or section boundary, every event uses its slot's single resolved root pitch, Stage 6A1 continuity is preserved between slots, outputs and events are frozen, inputs are unchanged, repeated calls are canonical-value identical, and changing only seed/provenance does not change events. Runtime invalid rhythm identifiers must produce `INVALID_BASS_RHYTHM`; malformed or impossible projections must produce `INVALID_BASS_TIMING`. No implementation evidence is claimed by this contract milestone.
