...
The Stage 7B4 implementation must prove that the two-argument call and every existing complete rate/direction-only call remain canonical-value equivalent to an explicit full-step gate; `gateTicks === rateTicks` preserves Stage 7B3 at quarter, eighth, and sixteenth rates; and representative shortened gates at all three rates change only duration. It must assert unchanged pitch order, event count, start ticks, slot-local reset, and section containment; positive duration no greater than the selected rate; no event crossing its next step, slot, or section; same-pitch adjacent-step separation; immutable output and input non-mutation; equal-input replay; and ambient-randomness isolation. Runtime cases must cover the inclusive boundaries `1` and `rateTicks`, treat an absent property and explicit `undefined` identically as the full-step default, and reject zero, negatives, over-rate integers, fractions, `NaN`, both infinities, unsafe integers, strings, nulls, and other forged values as `INVALID_ARP_GATE` at `parameters.gateTicks`. Mixed-invalid cases must freeze range, Harmony context, `NO_LEGAL_ARP_PITCH`, rate, direction, gate, then internal-timing precedence. Regression coverage must retain all Stage 7B1/B2/B3 behavior; no test may claim gate ratios, percentages, overlap, velocity, MIDI articulation, octave, density, seed, or profile-policy behavior.
