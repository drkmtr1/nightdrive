...
### First successor profile-data ownership — compatibility accepted, R1 candidate review-pending

The successor compatibility direction in [ADR-019](DECISIONS.md) is accepted and merged through PR #109. It reserves `nightdrive.genre-profile.arpeggiator.v2` for a new immutable calibration of the same four profile IDs: `dark-synthwave`, `classic-synthwave`, `darkwave`, and `midtempo-cyberpunk`. It is paired only with `nightdrive.arpeggiator-policy.v2` under the exact compatibility matrix in the [Arpeggiator model](ARPEGGIATOR_MODEL.md). Neither identity is implemented. The exact numerical R1 candidate and research fingerprint are documented separately in the [Stage 7 V2 calibration record](reviews/STAGE7_ARPEGGIATOR_V2_CALIBRATION.md) for review; that record is an ordered research representation, not caller-supplied runtime configuration or a canonical persistence format. The V1 records above remain literal, immutable, and replay-authoritative.
...
The R1 candidate adjusts Complexity rows across all four profiles and Energy rows in Dark Synthwave, Classic Synthwave, and Midtempo Cyberpunk. Darkwave's medium-Complexity Energy path is preserved as a positive control. The candidate record supplies all 500 effective lists by exact construction and documents its retained regressions. Literal runtime validation, exact V2 public request/result/error ownership, V1 replay isolation, and controlled comparative listening remain separate gates; no V2 musical acceptance or runtime implementation is claimed.
