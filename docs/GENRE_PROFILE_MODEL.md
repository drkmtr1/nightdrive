...
### First successor profile-data ownership — review-pending

The product-owner-approved successor direction in [ADR-019](DECISIONS.md) reserves `nightdrive.genre-profile.arpeggiator.v2` for a new immutable calibration of the same four profile IDs: `dark-synthwave`, `classic-synthwave`, `darkwave`, and `midtempo-cyberpunk`. It is paired only with `nightdrive.arpeggiator-policy.v2` under the exact compatibility matrix in the [Arpeggiator model](ARPEGGIATOR_MODEL.md). Neither identity is implemented, and no V2 table or numeric weight is defined here. The V1 records above remain literal, immutable, and replay-authoritative.

For every existing profile/decision slot, the first successor must retain V1 candidate membership and declaration order. Within those fixed lists, profile-data V2 may define new base integer weights, five-level Energy vectors, and five-level Complexity-addition vectors, including nonzero Energy or Complexity influence on an existing slot that was invariant in V1. It must retain the accepted independent row selection and exact candidate-aligned integer addition, Stage 7C2 weight/total bounds, deterministic ordering, no normalization or candidate removal, and recursively immutable data. This permission is calibration scope, not a value proposal. Changing subsets or order is deferred to a separate contract decision; no new profile, slot, candidate domain, or musical dimension is included.

Complexity calibration is initially in scope for all four profiles. Energy calibration is initially in scope for Dark Synthwave, Classic Synthwave, and Midtempo Cyberpunk. Darkwave Energy is the initial positive-control reference rather than an automatic tuning target; no requirement to copy every V1 number into V2 is implied before evidence-based calibration. A separately authorized calibration contract must freeze exact V2 records, weights, vectors, literal validation bounds, and deterministic candidate-list evidence before runtime implementation. Controlled comparative listening and V1 replay isolation remain later acceptance gates, not results claimed by this checkpoint.
