...
The R1 dataset adjusts Complexity rows across all four profiles and Energy rows in Dark Synthwave, Classic Synthwave, and Midtempo Cyberpunk. Darkwave's medium-Complexity Energy path is preserved as a positive control. The candidate record supplies all 500 effective lists by exact construction and documents its retained regressions. Its arithmetic, hash, structure, and permitted-delta evidence were independently verified; collision totals are now reproduced by the review-pending retained diagnostic; historical search/finalist chronology remains unreproduced. Reproducible deterministic diagnostics, literal runtime validation, V1 replay isolation, and controlled comparative listening remain separate gates; the exact V2 public request/result/error ownership is accepted through PR #111, while no V2 musical acceptance or runtime implementation is claimed.
