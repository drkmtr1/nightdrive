...
The initial Stage 7 audition-artifact route is frozen in [Stage 7 Arpeggiator audition-artifact contract](reviews/STAGE7_ARPEGGIATOR_AUDITION_ARTIFACT.md): an evaluation-only assembler maps accepted Harmony plus Arp output into the existing Nightdrive MIDI IR and serializer for manual FL Studio audition. This route does not require or implement browser preview; exact instruments, mix/effects, import settings, presentation order, Energy/Complexity matrix, and additional seeds remain separate listening-protocol decisions.
