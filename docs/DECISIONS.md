...
**Date:** 2026-09-14
**Status:** Accepted for the Stage 7C documentation-only architectural boundary. Stage 7C1 mask and Stage 7C2 weighted-choice contracts are accepted and merged; Stage 7C3 exact seed-derivation arithmetic, byte layout, and golden vectors are under documentation review. Runtime implementation remains separately gated.
...
**Consequences:** Stage 7C3 proposes the identifier `nightdrive.seed-derivation.component.v1` and an exact dependency-free MurmurHash3 x86_32 derivation over a domain-separated canonical byte layout, with normative cross-runtime vectors in the Arpeggiator model. Production use remains prohibited until that contract is reviewed and accepted. Arpeggiator policy order and profile data receive independent versions; changing any replay-relevant boundary requires a new appropriate version. No new dependency is warranted for this small non-cryptographic mechanism.
