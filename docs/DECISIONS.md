...
**Date:** 2026-09-14
**Status:** Accepted for the Stage 7C documentation-only architectural boundary. Exact seed-derivation arithmetic, byte layout, golden vectors, weighted-choice mechanics, mask catalogs, and runtime implementation remain separately gated.
...
**Consequences:** Stage 7C may document the proposed identifier `nightdrive.seed-derivation.component.v1` and a dependency-free MurmurHash3 x86_32-style candidate, but production use is prohibited until exact constants, operation ordering, byte encoding/layout, supported component IDs, and golden vectors are reviewed and frozen. Arpeggiator policy order and profile data receive independent versions; changing any replay-relevant boundary requires a new appropriate version. No new dependency is warranted for this small non-cryptographic mechanism.
**Revisit:** Reconsider component isolation only if deterministic integration evidence shows it cannot preserve replay or locked-component independence; reconsider the proposed derivation algorithm before acceptance if exact-vector or cross-runtime evidence reveals ambiguity or mismatch.
