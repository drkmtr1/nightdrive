...
**Decision:** Define a Nightdrive-owned, versioned MIDI intermediate representation over validated 960-PPQ integer composition events. Map it to Standard MIDI File format 1 through an isolated adapter with fixed conductor/component tracks, channels, event ordering, note lifecycle, and structured failures. Require independent parsing and binary fixtures for later implementation validation; parsing is test/reference-only in Stage 5A.
