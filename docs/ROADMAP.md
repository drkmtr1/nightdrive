...
**Current bounded slice:** Stage 7C1 is accepted and merged through PR #72 (approved head `8d0a2d5ba3919e64b96cc2267cab0a0efd66d75e`, merge commit `77fde7d939f8da563cfbff28f4c5e69c37d754b2`). Stage 7C2 is a documentation-only deterministic weighted-choice contract under review; it proposes exact ordered-candidate, integer-weight, total, one-uint32 modulo mapping, half-open bucket, zero-weight, single-candidate, validation, and versioning semantics without profile weights, seed derivation, public errors, or runtime implementation.
...
**Pre-implementation gates:** Accept the Stage 7C2 deterministic integer-weighted-choice contract currently under review; freeze `nightdrive.seed-derivation.component.v1` constants, uint32 operations, canonical bytes, supported identifiers, and golden cross-runtime vectors; exact profile candidates, weights, and gate candidates; and structured error taxonomy/precedence. The MurmurHash3 x86_32-style derivation remains provisional until those vectors are accepted.
...
**Evidence before completion:** Stage 7B compatibility; octave `2`/`3` bounds/deduplication/order; exact mask behavior and traversal consumption; equal-input/seed/version replay; component-seed isolation; fixed decision-slot consumption; ambient-independence; profile-bound enforcement; seed-derivation golden/cross-runtime vectors; AC-004/AC-011/AC-013 evidence; and structured human listening evaluation for all four profiles.
