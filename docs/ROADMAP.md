...
**Current prerequisite contract:** Stage 7C-P1 is a documentation-only definition of the shared normalized composition-brief `EnergyV1` and `ComplexityV1` domains. It freezes the exact five identifiers, ordinal order, orthogonality, `medium` creation defaults, strict canonical validation, and composition-brief schema/replay ownership. It does not define profile candidates, weights, gate candidates, Stage 7C errors, or runtime behavior; Stage 7C4 remains separately gated and unstarted.
...
**Pre-implementation gates:** Freeze exact profile candidates, weights, and gate candidates; and freeze structured error taxonomy/precedence. Any further Stage 7C contract slice requires separate authorization.
