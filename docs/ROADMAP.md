...
**Current bounded contract:** Stage 7C4 is documentation-only and freezes `nightdrive.arpeggiator-policy.v1`, the `nightdrive.genre-profile.arpeggiator.v1` profile-data set, exact profile candidate order and raw integer weights for all five decision slots, explicit energy/complexity lookup construction for all 25 pairs, and exact integer `short|medium|long` gate resolution. It does not authorize runtime implementation or freeze Stage 7C structured errors.
...
**Pre-implementation gates:** Review and accept Stage 7C4, then separately freeze structured-error taxonomy, fields, and precedence. Runtime implementation remains separately gated after those contracts.
