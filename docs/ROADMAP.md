...
**Current gate:** The corrected listening setup, external 28-fixture package, locked Pass 1/Pass 2 evidence, and deterministic policy-sensitivity diagnostic are recorded in [Stage 7 Arpeggiator evaluation results](reviews/STAGE7_ARPEGGIATOR_EVALUATION_RESULTS.md). No runtime defect was identified; individual usability and seed stability passed for the exploratory single-reviewer baseline, while Energy and Complexity sensitivity remain REVISE/open. The product owner approved a new immutable profile-data identity paired with a new compatibility/policy identity under the unchanged five-slot algorithm; this documentation checkpoint is review-pending. The next eligible task after its acceptance is a separately authorized exact V2 profile-data calibration contract, not runtime implementation or Stage 8.
...
**Status:** Product-owner architecture direction approved; documentation contract review-pending. [ADR-019](DECISIONS.md), the [Arpeggiator model](ARPEGGIATOR_MODEL.md), and the [genre-profile model](GENRE_PROFILE_MODEL.md) define the exact V1/V2 compatibility pairs and preserve V1 replay. The first successor keeps the same five-slot algorithm, seed/PRNG/selector/projector, shared domains, and every profile's candidate subset/order; only profile-owned weights and Energy/Complexity vectors may change. No V2 weights or runtime are defined. A separately authorized calibration contract must freeze exact data and deterministic vectors before a distinct V2 public boundary can be implemented. Comparative human listening remains later; Stage 7 is open and Stage 8 is not authorized.
