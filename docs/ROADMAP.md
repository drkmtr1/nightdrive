...
**Current bounded slice:** Stage 7C1 is a documentation-only deterministic density/rest-mask contract under review. It proposes the closed `nightdrive.arp-density-mask.v1` catalog and exact step, repetition, reset, traversal-consumption, compatibility, representation, and versioning semantics without authorizing runtime implementation or another Stage 7C slice.
...
**Pre-implementation gates:** Accept the Stage 7C1 exact mask catalog and semantics currently under review; freeze deterministic integer-weighted-choice candidate order, totals, zero-weight and bucket boundaries; `nightdrive.seed-derivation.component.v1` constants, uint32 operations, canonical bytes, supported identifiers, and golden cross-runtime vectors; exact profile weights/gate candidates; and structured error taxonomy/precedence. The MurmurHash3 x86_32-style derivation remains provisional until those vectors are accepted.
