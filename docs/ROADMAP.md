...
**Status:** Compatibility architecture accepted and merged through PR #109 at merge commit `590075f45a4d5e03480b1d8c3a076d1119cb03e9`. [ADR-019](DECISIONS.md), the [Arpeggiator model](ARPEGGIATOR_MODEL.md), and the [genre-profile model](GENRE_PROFILE_MODEL.md) define the exact V1/V2 pairs and preserve V1 replay. The first successor keeps the same five-slot algorithm, seed/PRNG/selector/projector, shared domains, and every profile's candidate subset/order; only profile-owned Energy weights and Complexity additions may change. The exact [R1 calibration dataset](reviews/STAGE7_ARPEGGIATOR_V2_CALIBRATION.md) is accepted for dataset-only use at fingerprint `b6f7ee16f33cf649ae2c6f06e4b5eecf859409b1917e2bc641323857fc1956e8`; its arithmetic, hash, structure, and permitted-delta evidence were independently verified, while collision totals are now reproduced by the accepted retained diagnostic; historical search/finalist chronology remains unreproduced. [ADR-020](DECISIONS.md) accepts the two public-boundary decisions on additional properties and operation-local version checks; the complete [V2 public contract](ARPEGGIATOR_MODEL.md) is accepted and merged through PR #111 at approved head `f505dc7c14dc83b50c800986d883f7fe5d5da704` with merge commit `b3a42464ea44450bb017a1c65dd316d0d069c191`. The diagnostic and public V2 runtime are accepted through PR #114 and PR #131. The [Product Owner override](reviews/STAGE7_ARPEGGIATOR_EVALUATION_RESULTS.md#product-owner-r1v2-acceptance-and-comparison-override) accepts current R1/V2 behavior for product development on partial personal listening plus deterministic/prior evidence; it does not erase the original baseline findings or claim complete comparison evidence. Aggregate generator/provenance integration and Stage 7 exit evidence are accepted; Stage 7 is complete. The next gate is Product Integration / First Playable assessment; Stage 8 remains unauthorized.

### Post-Stage-7 Product Integration / First Playable assessment gate

Nightdrive has accumulated substantial deterministic engine capability while its integrated user-facing product experience remains comparatively immature. This is a sequencing concern, not a relaxation of engineering rigor: deterministic generation, canonical state, replayability, explicit contracts, structured errors, provenance/integrity, testing, and exact-head review remain required. After all existing Stage 7 exit requirements are satisfied, stop before Stage 8. A separately authorized, assessment/specification-only gate must determine the smallest architecturally valid vertical slice that lets the Product Owner generate, hear, and meaningfully evaluate useful Nightdrive music using accepted Stage 7 capabilities. The guiding principle is that every reasonable sequence of engineering milestones should periodically produce something the Product Owner can run, hear, see, or meaningfully evaluate. The exploratory baseline to assess is basic musical inputs → Generate → one 8-bar Harmony + Bass + Arpeggio section → Play/Stop/Loop; this is not frozen scope and does not authorize implementation.

The assessment must compare sequencing options and establish what accepted Harmony, Bass, Arpeggiator, composition, MIDI/time, and application-shell capabilities can be reused; whether a minimal coordinator is missing; the minimum inputs and visible generated state; the smallest audition behavior; whether any bounded Stage 9/browser-audition subset should move; whether Stage 8 Melody/Motif should precede, follow, or enhance First Playable; constraining contracts/dependencies; proportionate evidence; deferred capabilities; an ordered sequence beginning with First Playable and considering later musical expansion, bounded editing, locking/variation, persistence, DAW handoff, later intelligent/support capabilities, and release hardening; and clear Product Owner run/generate/hear/understand/evaluate exit criteria. That progression is a strategic hypothesis to compare against the existing roadmap and other viable sequences, not an accepted roadmap or authorized implementation order. It must preserve canonical deterministic architecture and must not assume browser audition, Stage 8, or any resequencing outcome.

This gate does not authorize roadmap resequencing, Stage 8 or Stage 9 implementation, browser audition, a composition coordinator, First Playable scope, architecture/scope/requirements/acceptance changes, or any other capability. Any resulting change requires a subsequent explicit Product Owner decision and bounded task. Stage 7 exit requirements remain unchanged; Stage 7 completes before this gate; Stage 8 remains unauthorized until its disposition is explicitly handled.
