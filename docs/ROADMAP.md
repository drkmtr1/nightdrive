...
**Capabilities:** Versioned Nightdrive-owned MIDI IR, validated canonical timing input, 960-PPQ integer ticks, Standard MIDI File format 1 conductor/component policy, fixed channels, deterministic note lifecycle and equal-tick ordering, structured failures, semantic/binary determinism distinction, isolated adapter interface, independent-parser validation plan, FL Studio acceptance protocol, and a bounded `midi-file` versus `midi-writer-js` dependency spike definition.
