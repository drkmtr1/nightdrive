...
This roadmap has no calendar promises. Each bounded stage becomes eligible only when current coordination state and accepted scope, decisions, contracts, and dependencies identify it as eligible; its exit evidence must pass before completion. Completing one stage does not by itself make the next capability eligible. A future-capable schema does not grant future behavior. Implementation stages must complete the Build vs. Buy / Dependency Evaluation Gate before custom commodity infrastructure or consequential dependencies are adopted.
...
**Accepted capabilities:** Rate, direction, range, gate, octave, density, seeded patterns, and concrete profile policy are implemented and accepted. Later musical capabilities remain separately gated.
**Dependencies:** Stages 3–4.
...
**Tests:** Active-Harmony derivation, selected-voicing compatibility, boundary/gate/range, exact direction cycles, slot reset, immutability, ambient-randomness isolation, replay, golden review, octave, density, seed, profile behavior, aggregate integrity, and pinned-runtime equivalence are covered by accepted evidence.
...
**Current gate:** The aggregate/provenance specification is accepted and merged through PR #143, its canonical foundation is accepted and merged through PR #144, the deterministic UTF-8/SHA-256 adapter is accepted and merged through PR #147, component canonical projections/component-hash construction are accepted and merged through PR #149, and aggregate `resultHash` construction with exact self-exclusion semantics is accepted and merged through PR #151 at merge commit `5cec539953490eee6dec5a9c0ad7c8a53d9f5f37`. Aggregate digest verification is accepted and merged through PR #153 at approved head `384ba17303a0c2d66b9fae815669d01d980c13da` with merge commit `273ec2a3c7b3692209188d2020f8f459f884117e`. Aggregate preflight/orchestration is accepted and merged through PR #155 at approved head `c4191ba0291f88affdd04e5fe0a441cfcb341eec` with merge commit `fe36802ffc9e86dde474cc8f7bf78e9eaca76091`. The independent AC-004 reference-vector derivation method and canonical Harmony snapshot capture/freeze method are accepted through PRs #158 and #159. The four canonical Harmony evidence snapshots are accepted and merged through PR #161 at reviewed head `c67295bde50b599caba90f0433352815ebac9bc7` with merge commit `bbb5ab9b93dbb4edf26bc42a325e2e455f41ca9c`. The exact 17-vector AC-004 oracle dataset is accepted/frozen through PR #167 at freeze merge commit `86a09f9e728b0aa331ff4d9efe89ca96ac865263`. Full pinned-Node AC-004 evidence, including Linux x64 execution and exact Windows/Linux cross-runtime equivalence, is accepted through PR #169 at reviewed head `46a46fa91f0b3bde1736a6fa0149d7648f2199d9` with merge commit `31df7efec12f314a061bfe17d66525e6b8c00b88`. Stage 7 exit-evidence reconciliation subsequently completed. The [Product Owner R1/V2 acceptance override](reviews/STAGE7_ARPEGGIATOR_EVALUATION_RESULTS.md#product-owner-r1v2-acceptance-and-comparison-override) remains accepted through PR #142. AC-011/AC-013 remain satisfied, R1-REV-001 remains partially closed, MIA-003 remains deferred/non-blocking, Stage 7 is complete, and Stage 8 remains ineligible pending its product/roadmap disposition.
...
**Non-goals:** Aggregate runtime during this specification task; Harmony generation; AI-authored or arbitrary chromatic notes; chord/scale additions; Harmony replacement or new voicing authority; floating per-step probability; velocity/accent, swing, humanization, MIDI articulation, browser/audio, UI, persistence, arbitrary scripting, or dependency installation.
**Evidence before completion:** Stage 7B compatibility; octave `2`/`3` bounds/deduplication/order; exact mask behavior and traversal consumption; every Stage 7C4 candidate order and final raw weight across all 25 energy/complexity pairs; all exact gate mappings; equal-input/seed/version replay; component-seed isolation; fixed decision-slot consumption; ambient-independence; profile-bound enforcement; seed-derivation golden/cross-runtime vectors; AC-004/AC-011/AC-013 evidence; and structured human listening evaluation for all four profiles. The [current R1 Product Owner exception](reviews/STAGE7_ARPEGGIATOR_EVALUATION_RESULTS.md#product-owner-r1v2-acceptance-and-comparison-override) waives completion of its matched listening procedure only; it does not claim four-profile coverage from the 35 assessed fixtures or waive any deterministic/technical exit obligation.
**Exit:** AC-011, AC-004, and AC-013 pass in full. These criteria are satisfied by accepted runtime, aggregate, pinned-runtime, and Product Owner R1/V2 evidence. The remaining matched-listening procedure is waived/superseded for this bounded acceptance and does not block completion.

### Stage 7 successor versioning and compatibility checkpoint

**Status:** Compatibility architecture accepted and merged through PR #109 at merge commit `590075f45a4d5e03480b1d8c3a076d1119cb03e9`. [ADR-019](DECISIONS.md), the [Arpeggiator model](ARPEGGIATOR_MODEL.md), and the [genre-profile model](GENRE_PROFILE_MODEL.md) define the exact V1/V2 pairs and preserve V1 replay. The first successor keeps the same five-slot algorithm, seed/PRNG/selector/projector, shared domains, and every profile's candidate subset/order; only profile-owned Energy weights and Complexity additions may change. The exact [R1 calibration dataset](reviews/STAGE7_ARPEGGIATOR_V2_CALIBRATION.md) is accepted for dataset-only use at fingerprint `b6f7ee16f33cf649ae2c6f06e4b5eecf859409b1917e2bc641323857fc1956e8`; its arithmetic, hash, structure, and permitted-delta evidence were independently verified, while collision totals are now reproduced by the accepted retained diagnostic; historical search/finalist chronology remains unreproduced. [ADR-020](DECISIONS.md) accepts the two public-boundary decisions on additional properties and operation-local version checks; the complete [V2 public contract](ARPEGGIATOR_MODEL.md) is accepted and merged through PR #111 at approved head `f505dc7c14dc83b50c800986d883f7fe5d5da704` with merge commit `b3a42464ea44450bb017a1c65dd316d0d069c191`. The diagnostic and public V2 runtime are accepted through PR #114 and PR #131. The [Product Owner override](reviews/STAGE7_ARPEGGIATOR_EVALUATION_RESULTS.md#product-owner-r1v2-acceptance-and-comparison-override) accepts current R1/V2 behavior for product development on partial personal listening plus deterministic/prior evidence; it does not erase the original baseline findings or claim complete comparison evidence. Aggregate generator/provenance integration and Stage 7 exit evidence are accepted; Stage 7 is complete. The Product Owner-authorized First Playable canonical composition specification and deterministic runtime are accepted and merged through PR #173 and PR #175 respectively. Its source, reference, and oracle evidence are accepted/frozen. The reviewed Linux harness and custody verification are integrated through PR #212, and Linux x64 qualification passed; the supported local Windows ARM64 member and exact cross-platform comparison remain the next execution gate. Stage 8 remains ineligible pending its product/roadmap disposition.

### Post-Stage-7 Product Integration / First Playable integration gate

Nightdrive has accumulated substantial deterministic engine capability while its integrated user-facing product experience remains comparatively immature. This is a sequencing concern, not a relaxation of engineering rigor: deterministic generation, canonical state, replayability, explicit contracts, structured errors, provenance/integrity, testing, and exact-head review remain required. After all existing Stage 7 exit requirements are satisfied, stop before Stage 8. The completed assessment selected a local in-memory one-8-bar Harmony+Bass+Arpeggiator generation boundary as the smallest deterministic product integration. Product Owner ADR-023 selects a new versioned canonical result for that boundary; the specification and deterministic runtime are accepted and merged through PR #173 and PR #175 respectively. Accepted/frozen source, reference, and oracle evidence plus the reviewed harness supported the passing Linux x64 qualification integrated through PR #212; the supported local Windows ARM64 member and exact cross-platform comparison remain the next execution gate. The guiding principle remains that every reasonable sequence of engineering milestones should periodically produce something the Product Owner can run, hear, see, or meaningfully evaluate.

The assessment compared sequencing options and established that accepted Harmony, Bass, and Arpeggiator can support a local in-memory eight-bar Harmony+Bass+Arpeggiator generation boundary, while browser audition remains downstream. The Product Owner selected a separately versioned canonical result/coordinator specification in ADR-023; its implementation is accepted and merged through PR #175. The accepted/frozen source, reference, and oracle evidence remain binding. Linux x64 qualification passed through the reviewed harness integrated in PR #212; the supported local Windows ARM64 member and exact cross-platform comparison remain the next execution gate. Browser audition, Stage 8, Stage 9, and later product sequencing remain ineligible without their own accepted scope and prerequisites.

This gate does not make browser audition, Stage 8 or Stage 9 implementation, playback/audio scope, UI, persistence, or roadmap resequencing eligible. The accepted coordinator/result implementation remains bounded to the local deterministic runtime. Its source, reference, and oracle evidence are accepted/frozen; Linux x64 qualification passed, while the supported local Windows ARM64 member and exact cross-platform comparison remain separately gated and pending. Stage 7 exit requirements remain unchanged and Stage 7 is complete. Stage 8 remains ineligible until its product/roadmap disposition is resolved.
